...
const Navbar = () => {
  ...
  return (
    ...
    <div className="hidden sm:flex items-center gap-8">
       ...
       {!user ? (<button onClick={()=>setShowUserLogin(true)} className="cursor-pointer px-8 py-2 bg-primary-dull transition text-white rounded-full">
            Login
        </button>)
        :
        (
  ...
  {open && (
   <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
        ...
        <NavLink to="/products" onClick={()=>setOpen(false)}>All prodcts</NavLink>
        {user&&
